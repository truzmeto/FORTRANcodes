...
program ContactMap
  ...
  integer:: count, count11, count12, count22, q_thresh
  integer:: q, q12, step_save
  integer:: i, j, ii, l, jj, i1, ll, id, n_cap, n_esc, state
...
!!##------------------------ here we calculate # of distinct configurations ---------------------------------
     ...
     if(q.ne.0) write(*,'(i3,2x,77(i2,1x))') n_esc, (icontact(i), i = 1, ncon12)
